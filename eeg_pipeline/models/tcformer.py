# Offline TCFormer implementation used by revision experiments.
...
class ChannelGroupAttention(nn.Module):
    """Official TCFormer group-wise squeeze-and-excitation attention."""
    ...
    def __init__(self, in_channels, num_groups, reduction=4):
        ...
        if reduced_channels % num_groups != 0:
            raise ValueError("Reduced channels must be divisible by num_groups")
    ...
    def forward(self, x):
        if x.shape[1] != self.in_channels:
            raise ValueError(
                f"Expected {self.in_channels} feature channels, got {x.shape[1]}"
            )
        weights = self.sigmoid(self.att_fc2(self.relu(self.att_fc1(self.pool(x)))))
        batch, _, height, width = x.shape
        grouped = x.view(batch, self.num_groups, self.group_size, height, width)
        weighted = grouped * weights.view(batch, self.num_groups, 1, 1, 1)
        return weighted.view_as(x)
...
class MultiKernelConvBlock(nn.Module):
    def __init__(
        ...
        self.use_group_attn = self.n_groups > 1 and use_group_attn
...
def _glorot_weight_zero_bias(module):
    """Match the initialization helper used by the official TCFormer code."""

    for layer in module.modules():
        if hasattr(layer, "weight") and "norm" not in layer.__class__.__name__.lower():
